...
const CarDetails = () => {
  ...
  const financeFee =
    (carPrice - depositAmount) *
    (monthlyInterestRate /
      (1 - Math.pow(1 + monthlyInterestRate, -loanDurationMonths)));

  const totalAmountPayable =
    financeFee * loanDurationMonths + depositAmount + optionToPurchaseFee;

  const totalInterestPaid = totalAmountPayable - carPrice;
  const annualPercentageRate =
    (totalInterestPaid / carPrice / (loanDurationMonths / 12)) * 100;
  return (
    <>
      <Navbar />
      <div className='car-details-wrapper'>
        ...
        <div className='car-details'>
          <div>
            ...
            >
              -£350 deposit upon any visa purchase{' '}
            </p>
          </div>
          ...
          <div className='car-details-table'>
            <table>
              <tbody>
                <tr>
                  <td>Finance Fee</td>
                  <td style={trStyle}>£{financeFee.toFixed(2)}</td>
                </tr>
                <tr>
                  <td>Option to purchase fee</td>
                  <td style={trStyle}>£{optionToPurchaseFee}</td>
                </tr>
                <tr>
                  <td>Total Amount Payable</td>
                  <td style={trStyle}>£{totalAmountPayable.toFixed(2)}</td>
                </tr>
                <tr>
                  <td>Duration</td>
                  <td style={trStyle}>{loanDurationMonths} Months</td>
                </tr>
              </tbody>
            </table>
            ...
            <p>
              <strong style={{ fontSize: '1rem', padding: '0.5rem' }}>
                {' '}
                {annualPercentageRate.toFixed(2)}% APR
              </strong>
          ...
          <div className='car-details-button'>
            <Link
              to='/reservationForm'
              state={{ carDetails: carDetails, cars: cars }}
            >
              <button>
